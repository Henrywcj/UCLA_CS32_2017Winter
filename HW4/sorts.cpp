...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    for (int i = 1; i < s.size(); i++)
    {
      ...
      int l = i;
      while (l > 0 && comp(nextStudent, s[l-1]))
      {
        s[l] = s[l-1];
        l--;
      }
...
int main()
{
      ...
      // destruction time.
    {
     ...
     vector<Student*> stuPtr;
     for (int i = 0; i < anxStudents.size(); i++)
      stuPtr[i] = &auxStudents[i];
      // TODO:  Sort the vector of pointers using the STL sort algorithm
      //        with compareStudentPtr as the ordering relationship.
     sort(stuPtr.begin(), stuPtr.end(), compareStudentPtr);
